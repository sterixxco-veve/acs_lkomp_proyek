...
export function Sidebar({ user, activeMenu, setActiveMenu, onLogout }) {
  ...
  if (user.role_name.includes('Admin') && user.role_name !== 'SuperAdmin') {
    menuItems.push('Dashboard', 'Master PC', 'Components', 'Software', 'Maintenance', 'Reports')
  }

  if (user.role_name === 'Sekretaris') {
    menuItems.push('Surat Peminjaman', 'Master Peminjam')
  }
          ...
          {menuItems.map((item) => (
            <button
              ...
              className={`w-full text-left px-4 py-3 rounded-xl transition-all ${
                activeMenu === item ? 'bg-[#5D7CEB]' : 'hover:bg-white/10'
              }`}
            >
              {item}
            </button>
